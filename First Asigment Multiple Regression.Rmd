...
#4  Given the response variable y and the covariates x2 and x3 in
...
modelo0=lm(data=Transform2_V2,y2~x2)
autoplot(modelo0)
```
...
```{r}
insurance=read_csv("Datasets_Chapters_1_to_4/insurance.csv")
head(insurance)
...
trainset$y2=BoxCox(trainset$charges,lambda)
crossvalidation$y2=BoxCox(crossvalidation$charges,lambda)
finalset$y2=BoxCox(finalset$charges,lambda)

#Best model so far ####
modelo_1_ajustar_y=lm(data=trainset,y2~smoker:age+highrisk+region+smoker)
...
qplot(y=trainset$bmi,x=trainset$age,color=trainset$residual>1)
