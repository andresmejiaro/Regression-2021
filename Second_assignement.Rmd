...
```{r,results=FALSE}
model_AIC=stepAIC(modall, k = 2, direction = "both", scope = . ~.^2)
model_BIC=stepAIC(modall, k = log(nrow(data_health)), direction = "both",scope = . ~ .^2)
```

```{r}
ltr=as.numeric(predict(modelo_actual,newdata = test,type = "response"))
#ltr=factor(ltr, levels = c(0,1))
test_response=factor(test$g02, levels = c(0,1))
aic=as.numeric(predict(model_AIC,newdata = test,type = "response"))
#aic=factor(aic, levels = c(0,1))
bic=as.numeric(predict(model_BIC,newdata = test,type = "response"))
#bic=factor(bic, levels = c(0,1))

# confusionMatrix(test_response, ltr)
# confusionMatrix(test_response,aic)
# confusionMatrix(test_response,bic)
...
For this exercise the models built by LTR and BIC gave the same result. The predictions of all the models are similar, at a cutoff of 0.5 gave exactly the same result. To further compare those models we used AIC on data not seen by the model before. The model generated by LTR and BIC (same model) is slightly better.
